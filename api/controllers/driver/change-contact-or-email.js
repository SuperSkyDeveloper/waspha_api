module.exports = {
  ...
  fn: async function (inputs, exits) {
    sails.log("action driver/change-contact-or-email started");
    ...
    try {
      ...
      if (is_email) {
        ...
        if (user_id == inputs.driver.email) {
          return exits.ok({
            status: false,
        ...
        other_user = await Driver.findOne(where);
        if (other_user) {
          return exits.ok({
            status: false,
        ...
        if (
          user_id.country_code == inputs.driver.country_code &&
          user_id.number == inputs.driver.contact
        ) {
          return exits.ok({
            status: false,
      ...
      sails.log("action driver/change-contact-or-email ended");
      ...
      sails.log(`Error in action driver/change-contact-or-email. ${err}`);
    }
  },
};
