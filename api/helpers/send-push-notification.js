...
module.exports = {
  ...
  inputs: {
    user: {
      type: "number",
      required: true,
      description: "A vendor|user|driver id",
    },
    role: {
      type: "string",
      required: true,
      //isIn: [_.values(global.ROLE)]
    },
    title: {
      type: "string",
    },
  ...
  fn: async (inputs, exits) => {
    ...
    switch (inputs.role) {
      case global.ROLE.VENDOR: {
        user = await Vendor.findOne({ id: inputs.user });
        break;
      }
      case global.ROLE.USER: {
        user = await User.findOne({ id: inputs.user });
        break;
      }
      case global.ROLE.DRIVER: {
        user = await Driver.findOne({ id: inputs.user });
        break;
      }
    }
    ...
    sails.log.debug(
      `send-push-notification ${inputs.role}_id: ${user.id}\nTitle: ${inputs.title}\nBody: ${inputs.body}`
    );
    // return exits.success();
    if (_.isEmpty(user.device_token)) {
      sails.log.debug(`userId: ${user.id} noisy notifications not permitted`);
      if (!inputs.silent) {
        sails.log.debug(`userId: ${user.id} not sending noisy notification`);
        return exits.success();
      }
    }
