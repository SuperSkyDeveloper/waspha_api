  ...
  fn: async function (inputs, exits) {
    ...
    switch (inputs.role) {
      case global.ROLE.VENDOR: {
        fb = sails.config.social.vendor.fb;
        break;
      }
      case global.ROLE.USER: {
        fb = sails.config.social.user.fb;
        break;
      }
      default: {
        fb = sails.config.social.vendor.fb;
    ...
    var urlToRq = `https://graph.facebook.com/oauth/access_token?client_id=${fb.app_id}&client_secret=${fb.secret_id}&grant_type=client_credentials`;
    ...
    request.get({ url: urlToRq }, function (err, response, body) {
      ...
      if (app_access_token) {
        ...
        request.get({ url: urlToRq }, function (err2, response2, body2) {
          if (err2) {
          ...
          if (!receivedData2.error) {
            ...
            if (fb_user_id) {
              ...
              request.get({ url: urlToRq }, function (err3, response3, body3) {
                if (err3) {
